	...
	if($_POST['news_title'] == NULL || $_POST['news_full'] == NULL){
		...
		if($_FILES['img']['name'] != NULL){
		   ...
		   if($_FILES['img']['type'] == "image/jpeg"
			...
			|| $_FILES['img']['type'] == "image/gif"){
			  if($_FILES['img']['size'] > 1048576){
			      ...
			      move_uploaded_file($tmp_name,$path.$name);
                          if($news->insert($title,$author,$full,$name,$date)){
                            header("location:".baseurl."admin/?page=news&act=list");
                            exit();
                          }else echo 'Khong the thêm vào cơ sở dữ liệu';
			  }
			}else{
			  // không phải file ảnh
			...
			//die("false");
                        if($news->insert($title,$author,$full,"",$date)){
                          header("location:".baseurl."admin/?page=news&act=list");
		          exit();
                        } else echo 'Khong the them vao co so du lieu';
		}
...
<fieldset>
  ...
  <tr>
    ...
    <td width="620"><input type="text" name="news_title" size="25"></td>
  </tr>
  <tr>
    <td>Tác giả</td>
    <td><input type="text" name="news_author" size="25"></td>
  </tr>
  <tr>
    <td>Chi tiết tin <span class="red">*</span></td>
    <td>
    <textarea cols="48" rows="6" class="ckeditor" name="news_full"></textarea>	
    </td>
  </tr>
  <tr>
    <td>Hình ảnh</td>
